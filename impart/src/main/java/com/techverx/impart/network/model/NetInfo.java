...
public class NetInfo {
    ...
    @Override
    public int hashCode() {
        int ip_custom = prefs.getBoolean(Constants.KEY_IP_CUSTOM, Constants.DEFAULT_IP_CUSTOM) ? 1:0;
        int ip_start = prefs.getString(Constants.KEY_IP_START, Constants.DEFAULT_IP_START).hashCode();
        int ip_end = prefs.getString(Constants.KEY_IP_END, Constants.DEFAULT_IP_END).hashCode();
        int cidr_custom = prefs.getBoolean(Constants.KEY_CIDR_CUSTOM, Constants.DEFAULT_CIDR_CUSTOM) ? 1:0;
        int cidr = prefs.getString(Constants.KEY_CIDR, Constants.DEFAULT_CIDR).hashCode();
        return 42 + intf.hashCode() + ip.hashCode() + cidr + ip_custom + ip_start + ip_end + cidr_custom + cidr;
    }

    public void getIp() {
        intf = prefs.getString(Constants.KEY_INTF, Constants.DEFAULT_INTF);
        try {
            if (intf == Constants.DEFAULT_INTF || NOIF.equals(intf)) {
            ...
            Log.e(TAG, e.getMessage());
            //Editor edit = Constants.edit();
            //edit.putString(Constants.KEY_INTF, Constants.DEFAULT_INTF);
            //edit.commit();
        }
        getCidr();
    }
